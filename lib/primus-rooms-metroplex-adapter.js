...
const DEFAULT_HEARTBEAT_PING_INTERVAL = 30000 // primus spark heartbeat default
...
module.exports = class PrimusRoomsMetroplexAdapter extends PrimusRoomsAdapter {
  constructor (redis, primus, opts = {}) {
    ...
    this._sparkRoomSetTTLSeconds = Math.ceil(
      (this.primus.options.pingInterval || DEFAULT_HEARTBEAT_PING_INTERVAL) / 1000 * TTL_REFRESH_DRIFT_FACTOR
    )
  }
  ...
  */
  broadcast (data, opts, clients, callback) {
    ...
    if (opts.rooms.length > 0) {
      ...
      // however, this is never actually called by our app, so it's not necessary to implement
